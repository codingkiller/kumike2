...
void NetworkBus::get_all_line(const QString city_id){
	...
	bool result;
	Q_UNUSED(result);
	result = connect(pNetworkAccessManager, SIGNAL(finished(QNetworkReply*)),this, SLOT(onAlllineDataModelFinished(QNetworkReply*)));
	Q_ASSERT(result);
	QNetworkRequest request = QNetworkRequest();
	m_city_id = city_id;
	request.setUrl(QUrl(get_lines_by_city_url+city_id));
	pNetworkAccessManager->get(request);
